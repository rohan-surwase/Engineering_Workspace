 ...
 * Handles: theme persistence, catalog discovery (via workspace-core.js),
 * card rendering, live search, category filtering, stats, and scroll
 * reveals. No frameworks, no build step, no external dependencies.
 */
(function () {
  "use strict";
  ...
  /* ---------------- Catalog state ---------------- */
  // Populated once GitHub discovery resolves. Everything else in this
  // file reads from here instead of a hand-maintained array.
  let CATALOG = [];

  function showLoading() {
    const wrap = document.getElementById("all-grid");
    if (wrap) wrap.innerHTML = '<div class="empty-state">Loading calculators…</div>';
  }
  ...
  function showCatalogError(err) {
    ...
    wrap.innerHTML = `
      <div class="empty-state">
        Couldn't reach the catalog.<br />
        <a href="#" id="catalog-retry">Try again</a>
      </div>`;
  ...
  async function loadCatalog(forceRefresh) {
    showLoading();
    try {
      const calcs = await window.EW.getCalculators({ forceRefresh: !!forceRefresh });
      CATALOG = calcs;
  ...
  function applyFilters() {
    ...
    if (countEl) countEl.textContent = `${filtered.length} of ${CATALOG.length}`;
  }
